...
export default function Progress() {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#080d08]">
        <div className="text-center space-y-3">
          <p className="text-[#d08888] text-sm">{error}</p>
          <a href="/" className="text-[#5a8a5a] hover:text-[#7aaa7a] text-xs transition-colors">← New Audit</a>
        </div>
      </div>
    );
  }
  ...
  if (!status) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#080d08]">
        <div className="w-5 h-5 border border-[#5a8a5a] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
    ...
    <div className="flex flex-col h-screen bg-[#080d08] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-[#141f14] shrink-0">
        <div className="flex items-center gap-4">
          <a href="/" className="text-[#5a8a5a] hover:text-[#7aaa7a] text-xs transition-colors">← New Audit</a>
          <span className="text-[#2a3a2a] text-xs">|</span>
          <span className="text-[#7aaa7a] text-xs truncate max-w-xs">{domain}</span>
        </div>
        {status.status !== "done" && status.status !== "error" && (
          <button
            onClick={handleTerminate}
            disabled={isTerminating}
            className="text-xs text-[#5a8a5a] hover:text-[#d08888] transition-colors disabled:opacity-40"
          >
      ...
      <div className="shrink-0" style={{ height: "72vh" }}>
        {activeUrl && !iframeBlocked ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-3">
            <p className="text-[#6a9a6a] text-xs">Preview blocked by site policy.</p>
            <a href={activeUrl} target="_blank" rel="noopener noreferrer"
              className="text-xs text-[#7aaa7a] hover:text-[#9acc9a] transition-colors">
              Open in new tab ↗
            </a>
          </div>
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            {status.status === "done" ? (
              <p className="text-[#5a8a5a] text-xs">Audit complete</p>
            ) : (
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 border border-[#5a8a5a] border-t-transparent rounded-full animate-spin" />
                <span className="text-[#5a8a5a] text-xs">Waiting for preview…</span>
              </div>
            )}
          </div>
        )}
      ...
      <div className="flex-1 border-t border-[#141f14] grid grid-cols-3 divide-x divide-[#141f14] overflow-hidden">
        ...
        <div className="overflow-y-auto px-5 py-4">
          <div className="text-[#5a8a5a] text-xs uppercase tracking-widest mb-3">
            Pages discovered
            {pagesDiscovered.length > 0 && (
              <span className="ml-2 text-[#7aaa7a]">
                ({pagesDiscovered.length}{status.total && status.total > 0 ? `/${status.total}` : ""})
              </span>
            )}
          </div>
          {pagesDiscovered.length === 0 ? (
            <p className="text-[#3a5a3a] text-xs">None yet</p>
          ) : (
            <div className="space-y-1.5">
              {pagesDiscovered.map((u, i) => (
                <div key={u} className="flex items-start gap-2 text-xs text-[#6a9a6a]">
                  <span className="text-[#3a5a3a] shrink-0 w-4 text-right">{i + 1}</span>
        ...
        <div className="overflow-y-auto px-5 py-4 bg-[#0a110a]">
          <div className="text-[#7ab07a] text-xs uppercase tracking-widest mb-4">Progress</div>
              ...
              return (
                <div key={phase.key} className="flex items-center gap-3">
                  <div className="shrink-0 w-5 flex items-center justify-center">
                    {done ? (
                      <span className="text-[#6a9a6a] text-xs">✓</span>
                    ) : active ? (
                      <div className="w-3 h-3 border border-[#7ab87a] border-t-transparent rounded-full animate-spin" />
                    ) : (
                      <div className="w-1.5 h-1.5 rounded-full bg-[#2a3a2a]" />
                    )}
                  </div>
                  <span className={`text-xs ${
                    done    ? "text-[#6a9a6a]" :
                    active  ? "text-[#a0caa0] font-medium" :
                              "text-[#3a4a3a]"
                  }`}>
                    {phase.label}
                  </span>
                  {active && status.progress !== undefined && status.total !== undefined && status.total > 0 && (
                    <span className="ml-auto text-[#6a9a6a] text-xs">
          ...
          {status.total !== undefined && status.total > 0 && status.progress !== undefined && (
            <div className="mt-5">
              <div className="h-px bg-[#1e2e1e] rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#5a8a5a] transition-all duration-500"
                  style={{ width: `${Math.round((status.progress / status.total) * 100)}%` }}
                />
              </div>
              <div className="text-[#6a9a6a] text-xs mt-1.5">
        ...
        {/* Col 3 — Activity */}
        <div className="overflow-y-auto px-5 py-4">
          <div className="text-[#5a8a5a] text-xs uppercase tracking-widest mb-3">Activity</div>
          <div className="space-y-4 text-xs">

            {status.agent_status && (
              <div>
                <div className="text-[#4a6a4a] uppercase tracking-widest mb-1.5">Agent</div>
                <p className="text-[#6a9a6a] leading-relaxed">{status.agent_status}</p>
              </div>
            )}

            {status.current_url && (
              <div>
                <div className="text-[#4a6a4a] uppercase tracking-widest mb-1.5">Current</div>
                <p className="text-[#6a9a6a] break-all leading-snug">{status.current_url}</p>
              </div>
            )}

            {status.result && status.result.pages.length > 0 && (
              <div>
                <div className="text-[#4a6a4a] uppercase tracking-widest mb-1.5">
                  Scanned ({status.result.pages.length})
                </div>
                <div className="space-y-1">
                  {status.result.pages.map((page, i) => (
                    <div key={i} className="flex justify-between gap-3 text-[#5a8a5a]">
            ...
            {!status.agent_status && !status.current_url && !status.result && (
              <p className="text-[#3a5a3a]">Waiting…</p>
            )}
          </div>
        </div>
